      ...
      <div className="relative mx-auto grid max-w-[1450px] gap-8 px-4 py-12 sm:px-6 lg:grid-cols-[0.9fr_1.1fr] lg:items-center lg:px-8">
        <div>
          ...
          <div className="mt-6 grid gap-3 sm:grid-cols-3">
            {["Ready to send", "Needs approval", "Human review recommended"].map((status) => (
              <GlassCard key={status} className="p-4" theme={theme}>
          ...
          {triageCards.map((card) => {
            ...
            return (
              <article
                key={card.label}
                className={cn(
                  "rounded-[1.35rem] border p-3 shadow-xl shadow-black/18",
              ...
              >
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    <p className="text-[0.68rem] font-black uppercase tracking-[0.14em] text-[color:var(--vs-accent-3)]">
                      {card.status}
                    </p>
