  ...
  return (
    <div className="cinematic-panel overflow-hidden rounded-[1.35rem] border border-white/16 bg-white/[0.06] shadow-[0_30px_120px_rgba(0,0,0,0.42)] backdrop-blur-2xl sm:rounded-[2.25rem]">
      <section className="relative overflow-hidden px-4 py-8 sm:px-9 sm:py-14 lg:px-11">
        <div className="absolute right-0 top-0 h-[28rem] w-[28rem] rounded-full bg-[#328BFF]/20 blur-3xl" />
        <div className="absolute bottom-0 left-10 h-64 w-64 rounded-full bg-[#6F4DFF]/14 blur-3xl" />
        <div className="relative grid gap-8 lg:grid-cols-[0.88fr_1.12fr] lg:items-center">
          <div>
            <Badge className="border border-[#8EBBFF]/22 bg-white/8 text-[#D7E8FF]">
              {business.heroBadge}
            </Badge>
            <h1 className="mt-5 max-w-3xl text-[2.15rem] font-semibold leading-[0.98] tracking-normal text-white sm:mt-6 sm:text-6xl">
              {business.headline}
            </h1>
            <p className="mt-5 max-w-2xl text-base leading-7 text-[#D7E2F7]/76 sm:text-lg">
      ...
      <section className="border-y border-white/10 bg-[#0B1024]/42 px-5 py-6 sm:px-9 lg:px-11">
            ...
            return (
              <div key={step.label} className="hover-lift relative rounded-2xl border border-white/12 bg-white/[0.05] p-4">
                <div className="flex items-center gap-3">
                  <div className="flex size-10 items-center justify-center rounded-xl bg-[linear-gradient(135deg,#328BFF,#6F4DFF)]">
                    <Icon className="size-5 text-white" aria-hidden="true" />
                  </div>
                  <div>
                    <p className="text-xs text-[#D7E2F7]/58">0{index + 1}</p>
      ...
      <section id="service-plans" className="grid gap-5 border-b border-white/10 px-5 py-8 sm:px-9 lg:grid-cols-[0.72fr_1.28fr] lg:px-11">
        <div>
          ...
          <p className="mt-3 text-sm leading-6 text-[#D7E2F7]/72">
        ...
        <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
          {business.services.slice(0, 6).map((service) => (
            <article key={service.title} className="hover-lift rounded-2xl border border-white/12 bg-white/[0.045] p-4">
              <Truck className="mb-4 size-5 text-[#6F4DFF]" aria-hidden="true" />
              <h3 className="font-semibold text-white">{service.title}</h3>
              <p className="mt-2 text-sm leading-6 text-[#D7E2F7]/70">{service.description}</p>
            </article>
          ))}
        </div>
      </section>
      ...
      <section className="grid gap-5 border-b border-white/10 px-5 py-8 sm:px-9 lg:grid-cols-2 lg:px-11">
        <div className="rounded-[1.5rem] border border-white/12 bg-white/[0.055] p-4 sm:p-5">
          ...
          <p className="mt-2 text-sm leading-6 text-[#D7E2F7]/72">{business.commonIssues.description}</p>
        ...
        <div className="rounded-[1.5rem] border border-[#6F4DFF]/20 bg-[#6F4DFF]/10 p-4 sm:p-5">
          <p className="text-xs font-semibold uppercase tracking-[0.18em] text-[#EAF1FF]">
          ...
          <div className="mt-5 grid gap-3">
            {business.process.steps.map((step, index) => (
              <div key={step.title} className="grid grid-cols-[2.2rem_1fr] gap-3 rounded-2xl border border-white/12 bg-[#0B1024]/46 p-3">
                <div className="flex size-9 items-center justify-center rounded-xl bg-white text-sm font-semibold text-[#0B1024]">
                  {index + 1}
                </div>
                <div>
                  <p className="font-semibold text-white">{step.title}</p>
                  <p className="mt-1 text-sm leading-5 text-[#D7E2F7]/70">{step.description}</p>
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="rounded-[1.5rem] border border-white/12 bg-white/[0.075] p-5 shadow-2xl shadow-black/18 sm:p-6">
          <div className="grid gap-5 lg:grid-cols-[1fr_auto] lg:items-center">
            <div>
              ...
              <p className="mt-2 max-w-2xl text-sm leading-6 text-[#D7E2F7]/76">
...
function DemoWorkflowPanel({ business }: { business: DemoBusinessConfig }) {
  ...
  return (
    <div className="relative">
      <div className="absolute -inset-5 rounded-[2rem] bg-[radial-gradient(circle_at_70%_20%,rgba(50,139,255,0.36),transparent_36%),radial-gradient(circle_at_40%_78%,rgba(111,77,255,0.18),transparent_34%)] blur-2xl" />
      <div className="cinematic-panel relative overflow-hidden rounded-[1.6rem] border border-white/14 bg-[#0B1024]/82 shadow-2xl shadow-black/25 backdrop-blur-2xl">
        <div className="flex items-center justify-between border-b border-white/10 px-4 py-3">
          <div>
            <p className="text-sm font-semibold text-white">Envo command workspace</p>
            <p className="text-xs text-[#D7E2F7]/52">{business.industry}</p>
          </div>
          <Badge className="border border-[#6F4DFF]/25 bg-[#6F4DFF]/12 text-[#EAF1FF]">Demo mode</Badge>
        </div>
        ...
        <div className="grid gap-3 p-4 pt-0 md:grid-cols-[1fr_0.85fr]">
          <div className="rounded-2xl border border-white/10 bg-white/[0.045] p-4">
            <p className="text-xs font-semibold uppercase tracking-[0.16em] text-[#6F4DFF]">Incoming lead</p>
            <h2 className="mt-3 text-2xl font-semibold text-white">{primary?.title ?? "New request"}</h2>
            <p className="mt-3 text-sm leading-6 text-[#D7E2F7]/72">{primary?.description ?? business.qualificationPreview.subtitle}</p>
          </div>
          <div className="rounded-2xl border border-[#8EBBFF]/20 bg-[#328BFF]/10 p-4">
            <p className="text-xs font-semibold uppercase tracking-[0.16em] text-[#D7E8FF]">Internal note</p>
...
function AccountValuePanel({ business }: { business: DemoBusinessConfig }) {
  return (
    <div className="rounded-2xl border border-white/12 bg-white/[0.055] p-5">
      <div className="flex items-center gap-3">
        <div className="flex size-11 items-center justify-center rounded-xl bg-[linear-gradient(135deg,#328BFF,#6F4DFF)]">
        ...
        <div>
          ...
          <p className="text-sm text-[#D7E2F7]/58">{business.beforeAfter.eyebrow}</p>
        </div>
      </div>
      <p className="mt-4 text-sm leading-6 text-[#D7E2F7]/72">{business.beforeAfter.description}</p>
      {business.beforeAfter.visualCards ? (
        <div className="mt-5 grid gap-3 sm:grid-cols-3 lg:grid-cols-1 xl:grid-cols-3">
          {business.beforeAfter.visualCards.map((card) => (
            <div key={card.label} className="rounded-2xl border border-white/10 bg-[#0B1024]/62 p-4">
              <p className="text-xs font-semibold uppercase tracking-[0.14em] text-[#D7E2F7]/52">{card.label}</p>
              <p className="mt-2 text-2xl font-semibold text-white">{card.value}</p>
              <p className="mt-1 text-xs leading-5 text-[#D7E2F7]/62">{card.description}</p>
            </div>
          ))}
        </div>
      ) : null}
    ...
    <div className="rounded-2xl border border-white/12 bg-white/[0.055] p-5">
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-sm text-[#D7E2F7]/58">{business.selectorLabel}</p>
        </div>
      </div>
      <div className="mt-5 grid gap-3">
        {business.leadOpsPanel.items.slice(0, 4).map((item) => (
          <div key={item.title} className="rounded-xl border border-white/10 bg-[#0B1024]/62 p-4">
            <p className="font-semibold text-white">{item.title}</p>
            <p className="mt-1 text-sm leading-6 text-[#D7E2F7]/70">{item.description}</p>
          </div>
        ))}
      </div>
    </div>
      ...
      {cards.map((card) => {
        ...
        return (
          <div key={card.title} className="rounded-2xl border border-white/12 bg-white/[0.045] p-4">
            <Icon className="mb-4 size-5 text-[#6F4DFF]" aria-hidden="true" />
            <p className="font-semibold text-white">{card.title}</p>
            <p className="mt-2 text-sm leading-6 text-[#D7E2F7]/70">{card.description}</p>
          </div>
        );
      })}
    </div>
...
function Metric({ label, value }: { label: string; value: string }) {
  return (
    <div className="rounded-xl border border-white/10 bg-white/[0.055] p-3">
      <p className="text-xs text-[#D7E2F7]/58">{label}</p>
