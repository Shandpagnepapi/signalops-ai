# LeadOps Demo Walkthrough

## Goal Of The Demo

Show skeptical business owners that LeadOps is an operational system, not a chatbot gimmick. Keep the demo practical, concrete, and tied to booking outcomes.

## Demo Flow At A Glance

1. LeadOps homepage  
2. Apex Wheel Repair demo  
3. Missed Lead Checkup form  
4. Dashboard  
5. How-it-works page  
6. Closing pitch

## 1) LeadOps Homepage Walkthrough

Talk track:

"Most businesses do not have a lead volume problem. They have a response and follow-up execution problem. LeadOps closes that gap."

What to point out:
- Core promise: instant response, qualification, and booking flow.
- Problem framing: leads get missed after form submit, missed call, DM, or ad inquiry.
- Packages: clear path from starter to custom system.
- CTA: Free Missed Lead Checkup as low-friction next step.

## 2) Apex Wheel Repair Demo Walkthrough

Talk track:

"Here is the same concept inside a real local-service scenario."

Narrative to present step-by-step:

1. Customer submits wheel repair quote request:
   "A customer comes in for curb rash, bent wheel, cracked wheel, or refinishing."
2. AI identifies likely damage type:
   "The system parses service intent and urgency from plain language."
3. AI asks for photos if needed:
   "If details are missing, it requests the exact photos needed to quote safely."
4. AI flags urgent/bent/cracked wheel leads:
   "Safety-sensitive language is escalated for human review."
5. AI recommends mobile service or shop inspection:
   "Cosmetic mobile-fit requests are routed differently than structural concerns."
6. AI sends internal sales note:
   "Owner/rep gets summary, urgency, and next action without reading a full thread."
7. Dashboard updates automatically:
   "Pipeline, score, tags, and status reflect the new lead immediately."

## 3) Missed Lead Checkup Form Walkthrough

Talk track:

"This is how we turn interest into diagnosis and implementation scope."

What to point out:
- Form gathers business context, lead volume, tools, and bottlenecks.
- Submission creates a structured intake, not a vague contact request.
- Confirmation includes a practical preview and next steps.

## 4) Dashboard Walkthrough

Talk track:

"This is the owner/operator view after implementation."

Show:
- Lead score, priority, urgency, and recommended action.
- Pipeline movement from new to booked/won/lost.
- Lead detail card with customer message, AI summary, internal note, and tags.
- Timeline events:
  - Lead submitted
  - AI qualified
  - Customer reply prepared/sent
  - Owner alert prepared/sent
  - Follow-up sequence started

## 5) How-It-Works Page Walkthrough

Talk track:

"This explains the engine in plain terms for non-technical owners and sales teams."

Highlight:
- Lead capture from forms/calls/ads/DMs.
- Instant response and qualification.
- Scoring and routing.
- Booking handoff.
- Follow-up sequence.
- Dashboard visibility.

Use this page when someone says, "Can you explain exactly what your system does?"

## 6) Closing Pitch

Primary close:

"If this makes sense, the next step is a Free Missed Lead Checkup. We look at how you handle calls, texts, forms, DMs, and follow-up, then show where leads are getting missed, delayed, or forgotten."

Secondary close:

"We can start with a pilot around one lead source and one workflow. You get measurable wins before expanding."

## Common Objections During Demo

### "We already follow up manually."

"That is usually the issue. Manual works until volume rises, then consistency drops. We automate the repetitive layer so your team handles the right conversations faster."

### "Is this replacing my staff?"

"No. It improves staff output by handling response, qualification prep, and follow-up consistency."

### "Will this work with my current tools?"

"Yes. LeadOps is designed to integrate with existing forms, CRM, phone, email, SMS, and calendar workflows."
